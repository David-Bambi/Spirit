...
void Vertices::SetVertexAttrPtr() {
  if (_vertextype.test(0)) {
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, _stride,
                          (void *)_offset.pos);
    glEnableVertexAttribArray(0);
  }
  if (_vertextype.test(2)) {
    glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, _stride,
                          (void *)_offset.tex);
    glEnableVertexAttribArray(2);
  }
  if (_vertextype.test(3)) {
    glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, _stride,
                          (void *)_offset.col);
