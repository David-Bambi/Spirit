...
ShaderProg::ShaderProg(const char *vertexPath, const char *fragmentPath) {
  std::string vertexCode = this->ReadShaderCode(vertexPath);
  std::string fragmentCode = this->ReadShaderCode(fragmentPath);

  unsigned int vertex = ShaderProg::Compile(vertexCode, GL_VERTEX_SHADER);

  unsigned int fragment = ShaderProg::Compile(fragmentCode, GL_FRAGMENT_SHADER);
  int success;
  char infoLog[512];
...
ShaderProg::~ShaderProg() { glDeleteProgram(this->_id); }

unsigned int ShaderProg::Id() { return this->_id; }

void ShaderProg::Id(unsigned int id) { this->_id = id; }

// use/activate the shader
void ShaderProg::Use() { glUseProgram(this->_id); }
...
unsigned int ShaderProg::Compile(std::string shaderCode,
